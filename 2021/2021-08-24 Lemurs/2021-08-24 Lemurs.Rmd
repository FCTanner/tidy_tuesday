...
dat %>% 
  filter(birth_type != "Unk",
         common_name != "hybrid") %>% 
  mutate(common_name = as.factor(common_name),
         concep_month = lubridate::month(as.Date(estimated_concep))) %>% 
  ggplot(aes(x = concep_month, y = reorder(common_name, concep_month, FUN = mean), group= common_name, color = concep_month)) +
